...
abstract class NetworkController<
  ...
  Future<void> _updateWeb3InternalChain(
  ...
  Future<void> _disconnectWeb3Chain(Web3ApplicationAuthentication app) async {
    await _storage.removeChainStorage(
        storage: DefaultChainStorageId.web3, key: app.applicationId);
  }

  Future<WEB3CHAIN> _getWeb3InternalChainAuthenticated(
  ...
  Future<List<WalletNetworkBackup>> _getNetworksBackup(
  ...
  Future<List<WalletBackupChainRepository>> _getChainBackup(
  ...
  Future<void> _restoreNetworksBackup(
  ...
  Future<void> _restoreChainBackup(
  ...
  Future<CONFIG> getConfig() async {
    final data =
        await _storage.queryChainStorage(storage: DefaultChainStorageId.config);
  ...
  E cast<E extends NetworkController>() {
    if (this is! E) {
      throw WalletExceptionConst.internalError("NetworkController.cast");
    }
    return this as E;
  }
}
